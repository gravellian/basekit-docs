...
* Requires its own lock file (composer.local.lock) so partial updates work.

Local Development WITH overrides
--------------------------------
* Always run Composer with the local file:
  COMPOSER=composer.local.json composer install --ignore-platform-req=php
  COMPOSER=composer.local.json composer update gravellian/basekit -W --ignore-platform-req=php
* site-update-basekit_host.sh already sets COMPOSER to composer.local.json.
* After editing ../basekit*, run the script or the update command above, then rebuild caches.

Local Development WITHOUT overrides
-----------------------------------
* Temporarily rename the file (mv composer.local.json composer.local.json.bak) or clear the COMPOSER env.
...
Quick Reference
---------------
* Local w/ overrides: COMPOSER=composer.local.json composer ...
...
2. rm -f composer.local.lock
3. unset COMPOSER
4. composer install
5. lando drush cr
6. ../scripts/site-update-basekit_host.sh dev.justsomeguypainting.com2 (optional)
